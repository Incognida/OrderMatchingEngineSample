class OrderList(object):
    ...
    Order, we may need multiple Orders to fulfil a transaction. The
    ...
    def get_head_if_head_deleted(self):
        if self.head_order is None:
            return None
        return_value = self.head_order
        self.head_order = self.head_order.prev_order
        return return_value
    ...
    def remove_order(self, order):
        ...
        if next_order is not None and prev_order is not None:
            ...
            self.head_order = next_order    # The next order becomes the first order
                                            #  in the OrderList after this Order is removed
        elif prev_order is not None:        # There is no next order
            prev_order.next_order = None
            self.tail_order = prev_order    # The previous order becomes the last order
                                            #  in the OrderList after this Order is removed
    ...
    def move_to_tail(self, order):
        ...
        if order.prev_order is not None:                # This Order is not the first Order in the OrderList
            order.prev_order.next_order = order.next_order      # Link the previous Order to the next Order,
                                                                #  then move the Order to tail
